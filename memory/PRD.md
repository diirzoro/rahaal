...
## v6.0 — «قاري رحّال» Smart Reader v1.0 (Tickets + Visas + Usage Credits + Super Admin) — DONE, backend tested 18/19
- طريقة إدخال ثالثة (يدوي | Excel | ✨ قاري رحّال) داخل شاشتي التذاكر والتأشيرات — لا شاشة مستقلة، لا Financial Engine جديد. القراءة تعبّئ النموذج الحالي فقط، والحفظ بمسار POST /tickets|/visas الحالي بكل الـ validators.
- محرك القراءة: /app/lib/smartReader.js — Provider-independent (env: SMART_READER_PROVIDER=gemini, SMART_READER_MODEL=gemini-3.6-flash, EMERGENT_LLM_KEY). المسار: Text PDF → استخراج نص محلي (pdfjs-dist legacy) → LLM نصي رخيص؛ Scanned PDF → base64 في الذاكرة → PDF vision أصلي؛ JPG/PNG → base64 vision. temperature 0 + tool-call schema صارم (null للمجهول، لا اختراع، فصل passport/ticket/PNR/mobile). Total النهائي → sale_price فقط؛ Cost لا يُخمّن أبداً.
- Collections: smart_reader_config (global: trial_credits=10 قابل للتعديل، enabled_default)، smart_reader_credits (لكل مكتب: free/paid/consumed/enabled — فهرس فريد tenant_id)، smart_reader_usage (id=read_id فهرس فريد = idempotency، يحفظ الحقول المستخرجة للـ replay، لا يحفظ المستند)، smart_reader_adjustments (تدقيق كامل before/change/after/reason/admin).
- Exact Charging Point: خصم قراءة واحدة atomic (free ثم paid، $gt:0 guard) فقط عند usable_read (تذكرة: اسم + ≥2 من [رقم/PNR/تاريخ/خط سير/شركة/مبلغ]؛ تأشيرة: اسم + (جواز|رقم تأشيرة)) وإرجاعها 200 للتعبئة الفورية في النموذج. Retry/Refresh/Double-click بنفس read_id → duplicate:true بدون خصم ثانٍ. فشل/غير مدعوم/غير قابل للاستخدام/رصيد صفر = لا خصم (402 قبل أي استدعاء AI).
- Routes: GET /smart-reader/credits، POST /smart-reader/read (multipart، RBAC حسب mod_tickets/mod_visas، عزل tenant كامل حتى على read_id). أدمن خلف adminGate: /admin/smart-reader/{overview,config,toggle,adjust,history} — reason إلزامي، لا رصيد سالب، history بدون PII fields.
- الخصوصية: المستند يُعالج في الذاكرة فقط ولا يُخزن إطلاقاً (لا قرص، لا DB)؛ لا PII في اللوجات؛ history الأدمن بدون الحقول المستخرجة.
- الاقتصاديات (فعلية مقاسة، gemini-3.6-flash): text_pdf ≈ $0.0005، scanned_pdf ≈ $0.0006، image ≈ $0.0005 لكل قراءة → هامش ممتاز عند تسعير $10/1000 قراءة (تتبع أوتوماتيكي في overview.economics + est_cost_usd لكل usage).
- Frontend: /app/app/smart-reader.js (SmartReaderButton/Dialog + SmartReaderAdminPanel)؛ page.js: زر ✨ في TopBar الشاشتين + prefill prop في TicketDialog/VisaDialog + تبويب «✨ قاري رحّال» في PlatformOfficesHub. عداد الرصيد + تحذير انخفاض (≤3) + حجب واضح عند الصفر (لا يمس اليدوي/Excel). Bulk جاهز معمارياً (read_id لكل ملف) دون تنفيذ v1.0.
- next.config.js: serverExternalPackages += pdfjs-dist, emergentintegrations. Packages: emergentintegrations@4, pdfjs-dist@4.10.38 (Node 20 compatible).
- Testing: backend agent 18/19 (الوحيد الفاشل = خطأ في harness الاختبار وليس التطبيق) — شمل charging rules، idempotency متزامن، عزل tenants، RBAC، أدمن (سبب إلزامي/لا سالب/تدقيق)، وFinancial Regression: صفر قيود/أرصدة من القراءة. E2E متصفح: قراءة PDF فعلي عبّأ نموذج التذكرة كاملاً. لا Push، لا Deploy.
- ملاحظة: tenant التجربة owner@gmail.com عليه بيانات تجربة (رصيد مدفوع 100+ وقراءات) — بيئة Workspace فقط.
