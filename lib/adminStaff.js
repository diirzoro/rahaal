...
// ------------------------------- REALM CHECKS -------------------------------
export const isMainAdmin = (u) => !!u && u.role === 'super_admin' && !u.tenant_id
...
export async function adminStaffHandler(db, path, method, p, body, sess) {
  ...
  if (path === '/users' && method === 'GET') {
    const users = await db.collection('users').find({ $or: [{ role: 'admin_staff' }, { role: 'super_admin', tenant_id: null }] }).toArray()
