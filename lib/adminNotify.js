...
export const NOTIFICATION_TYPES = {
  ...
  // v3.96 — Batch 4 wired types
  dispute_event: { label: 'أحداث النزاعات', source: 'مركز النزاعات (Batch 4)', wired: true, default_priority: 'action', needs_action: true },
  // v4.3 — wired by the unified orders/commissions flows:
  order_new: { label: 'طلب جديد (مركز الطلبات)', source: 'platform_orders (v4.3)', wired: true, default_priority: 'action', needs_action: true },
  order_event: { label: 'حدث على طلب (اعتماد/رفض)', source: 'platform_orders (v4.3)', wired: true, default_priority: 'info', needs_action: false },
  manual_admin: { label: 'تنبيه يدوي من الإدارة', source: 'إنشاء يدوي بصلاحية (v4.3)', wired: true, default_priority: 'info', needs_action: false },
  fx_rate_change: { label: 'تغيير أسعار الصرف', source: 'إدارة العملات والصرف (Batch 4)', wired: true, default_priority: 'warning', needs_action: false },
}
...
export async function adminNotifyHandler(db, path, method, p, b, sess) {
  ...
  const am = path.match(/^\/([^/]+)\/(read|unread|archive|unarchive|handled|assign)$/)
  if (am && method === 'POST') {
    const doc = await db.collection('admin_notifications').findOne({ id: am[1] })
    if (!doc) return { error: 'الإشعار غير موجود', status: 404 }
    const uid = sess.user.id
    const act = am[2]
    // v4.3 — assign a notification to a manager/department (audited)
    if (act === 'assign') {
      const assigned = { assigned_to: b?.assignee ? String(b.assignee).slice(0, 120) : null, assigned_department: b?.department ? String(b.department).slice(0, 80) : null, assigned_by: sess.user.email, assigned_at: new Date() }
      await db.collection('admin_notifications').updateOne({ id: doc.id }, { $set: assigned })
      await logAudit(db, sess, 'notify_assign', { id: doc.id, title: doc.title }, { assigned_to: doc.assigned_to || null }, assigned, b?.reason || null)
      return { success: true }
    }
  ...
  // v4.3 — manual notification creation (permission-gated route; sent history is immutable —
  // corrections are a NEW corrective notification, never an edit/delete)
  if (path === '/create' && method === 'POST') {
    if (!b?.title || !String(b.title).trim()) return { error: 'عنوان التنبيه إلزامي' }
    const r = await emitAdminNotification(db, {
      type: NOTIFICATION_TYPES[b.type] ? b.type : 'manual_admin',
      title: b.title, body: b.body || '',
      priority: b.priority, tenant_id: b.tenant_id || null,
      ref_type: b.ref_type || null, ref_id: b.ref_id || null,
      dedupe_key: b.dedupe_key || null,
    })
    if (r.error) return { error: r.error }
    if (r.skipped === 'duplicate') return { success: true, duplicate: true, id: r.id }
    await logAudit(db, sess, 'notify_manual_create', { id: r.id, title: b.title }, null, { type: b.type || 'manual_admin' }, b.reason || null)
