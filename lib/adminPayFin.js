  ...
  if (oM) {
    ...
    if (oM[3] === 'status' && method === 'POST') {
      ...
      if (to === 'confirmed') {
        ...
        const others = await db.collection('users').countDocuments({ $or: [{ role: 'super_admin', tenant_id: null }, { role: 'admin_staff', admin_realm: true }], active: { $ne: false }, email: { $ne: order.created_by } })
