...
async function applyPreserveMigration(db, tenantId, by) {
  ...
  try {
    ...
    //    • party lines → the party's own L4 LEAF account_code (v3.88.4 — F-007/F-008
    //      unified rule: group accounts never receive direct postings; group code stays
    //      only as fallback for a party that somehow has no leaf code)
      ...
      const newLines = (je.lines || []).map(l => {
        ...
        if (l.party_id && ['box', 'client', 'supplier'].includes(l.party_type)) {
          if (l.party_type === 'box') {
            ...
            code = bx?.account_code || (bx?.type === 'bank' ? C.BANKS : C.CASHBOXES)
          } else if (l.party_type === 'client') {
            code = clientsById[l.party_id]?.account_code || C.CLIENTS
          } else {
            code = suppliersById[l.party_id]?.account_code || C.SUPPLIERS
          }
