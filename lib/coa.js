...
async function applyResetMigration(db, tenantId, by, opts = {}) {
  ...
  // unless the caller is the explicit owner endpoint that already wipes (opts.allowWipe).
  // v3.88.1 — BLOCKER-2: allowWipe is NEVER honored on production. DISABLE_AUTO_SEED=true
  // is the permanent live-server flag (/etc/rahaal.env) — when it is set, a tenant holding
  // ANY financial document can never be fully reset, regardless of caller options.
  const isProduction = process.env.DISABLE_AUTO_SEED === 'true'
  const wipeAllowed = !!opts.allowWipe && !isProduction
  if (audit.transaction_total > 0 && !wipeAllowed) {
    if (isProduction && opts.allowWipe) {
      throw new Error(`RESET blocked on PRODUCTION: tenant has ${audit.transaction_total} financial documents — full wipe is forbidden on the live server, use PRESERVE`)
    }
